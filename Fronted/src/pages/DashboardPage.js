...
function DashboardPage() {
    ...
    const [salesDistributionByProduct, setSalesDistributionByProduct] = useState([]); // New state for pie chart
        ...
        const fetchDashboardData = async () => {
            ...
            try {
                const [summaryRes, salesByProductRes, salesBySellerRes, salesOverTimeRes, salesDistributionRes] = await Promise.all([
                    ...
                    api.get('/dashboard/sales-distribution-by-product').catch(err => { console.error("Error fetching sales distribution by product:", err); return { data: [] }; }), // New API call
                ]);
                ...
                console.log("Sales Distribution Data for Pie Chart:", distributionData); // --- Added console.log ---
        ...
        <Box sx={{ p: 3 }}>
            ...
            <Grid container spacing={3} mb={4}>
                <Grid item xs={12} sm={6} md={3}>
                    <Paper elevation={3} sx={{ p: 3, textAlign: 'center', bgcolor: theme.palette.primary.light, color: theme.palette.primary.contrastText, borderRadius: 2 }}>
                        <Typography variant="h6">סה"כ מכירות</Typography>
                        <Typography variant="h4" sx={{ fontWeight: 'bold' }}>{summary?.totalSales || 0}</Typography>
                    </Paper>
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                    <Paper elevation={3} sx={{ p: 3, textAlign: 'center', bgcolor: theme.palette.secondary.light, color: theme.palette.secondary.contrastText, borderRadius: 2 }}>
                ...
                <Grid item xs={12} sm={6} md={3}>
                    <Paper elevation={3} sx={{ p: 3, textAlign: 'center', bgcolor: theme.palette.info.light, color: theme.palette.info.contrastText, borderRadius: 2 }}>
                ...
                <Grid item xs={12} sm={6} md={3}>
                    <Paper elevation={3} sx={{ p: 3, textAlign: 'center', bgcolor: theme.palette.success.light, color: theme.palette.success.contrastText, borderRadius: 2 }}>
